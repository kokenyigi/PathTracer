...
void Camera::TryInfluenceMovement( int axisIndex, int direction, int influence)
{
	int influenceVal = direction * influence;
	if(abs(motionMults[axisIndex] + influenceVal) < 2)
	{
		motionMults[axisIndex] += influenceVal;
	}
